...
static void test_v1()
{
	...
	std::cout << "✓ MemoryPool v1 test passed\n\n";
}
...
static void test_v2_dynamic_adjustment()
{
	...
	for (int i = 0; i < 20; i++) {
		...
		if (i == 0) std::cout << "  First allocation (should fetch from CentralCache)\n";
		if (i == 10) std::cout << "  Allocating more blocks (may use cached blocks)...\n";
	}
	...
	std::cout << "  ✓ Deallocated all blocks (should trigger recycle to CentralCache)\n";
	
	std::cout << "✓ Dynamic adjustment test passed\n\n";
}
...
static void test_v2_multiple_sizes()
{
	...
	std::cout << "✓ Multiple sizes test passed\n\n";
}
...
static void test_v2_stress_test()
{
	...
	std::cout << "  ✓ Completed " << iterations << " cycles in " << duration.count() << " μs\n";
...
static void test_v2_alignment()
{
	...
	for (size_t size : unalignedSizes) {
		...
		std::cout << "  ✓ Size " << size << " aligned correctly at " << ptr.value() << "\n";
